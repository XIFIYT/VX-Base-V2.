...
namespace VX
{
	 ...
	 * \return bool
	 */
	inline std::string g_Name = ("VX Base V2");
	inline std::string GetMenuPath()
	{
	...
	{
		if (GetForegroundWindow() == g_gtaPointers->m_GameWindow)
		{
...
#	define CONSTEXPR_JOAAT_IMPL(x) (::Core::ConstexprJooat<sizeof(x) - 1>((x), std::make_index_sequence<sizeof(x) - 1>()).GetHash())
