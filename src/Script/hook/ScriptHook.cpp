#include "Script/hook/scriptHook.hpp"
...
namespace VX
{
	...
	ScriptHook::~ScriptHook()
	{
		if (m_Program)
		{
			...
			{
				auto og_handler = g_gtaPointers->m_GetNativeHandler(g_gtaPointers->m_NativeRegistrations, hash);
	...
	void ScriptHook::Ensure()
	{
		...
		if (auto program = g_gtaPointers->m_ScriptProgramTable->find_script(m_ScriptHash))
		{
			if (program->is_valid())
			{
				HookInstance(program);
